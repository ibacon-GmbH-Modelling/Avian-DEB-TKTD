...
function [prdData, info, f_R, cV] = predict_Anas_platyrhynchos(par, data, auxData)
...
% Control
par_CS = [k_e, c_0, c_T, c_0s, b, h_0, conc.tWw_fe_dat11_0, Fs, t0_c, F]; % pack tox parameters + delay exposure
...
prdData.tWw_fe_dat11_0 =  LEHRCS_Ww(:,1).^3 + w_E/ mu_E/ d_E * (LEHRCS_Ww(:,2))+ w_E/ mu_E * (LEHRCS_Ww(:,4)+LEHRCS_Ww(:,5)); % g, wet weight over time

LEHRCS_N = LEHRCS(ismember(t1, tN_dat11_0(:,1)),:);
prdData.tN_dat11_0= LEHRCS_N(:,6);
cV.tcV_dat11_0= LEHRCS_N(:,7);
f_R.tf_R_dat11_0 =LEHRCS_Ww(:,9); 


% 500 ppm a.i.
par_CS = [k_e, c_0, c_T, c_0s, b, h_0, conc.tWw_fe_dat11_1, Fs, t0_c,F]; % pack tox parameters + delay exposure
[t1, LEHRCS] = call_LEHRCS([t], LEHRCS_0, options,  ingestion.tWw_fe_dat11_1,par_LEHR, par_CS);
LEHRCS_Ww = LEHRCS(ismember(t1, tWw_fe_dat11_0(:,1)),:); % Extract growth time points
prdData.tWw_fe_dat11_1 = LEHRCS_Ww(:,1).^3 + w_E/ mu_E/ d_E * (LEHRCS_Ww(:,2))+ w_E/ mu_E * (LEHRCS_Ww(:,4)+LEHRCS_Ww(:,5)); % g, wet weight over time

LEHRCS_N = LEHRCS(ismember(t1, tN_dat11_1(:,1)),:);
prdData.tN_dat11_1 = LEHRCS_N(:,6); 
cV.tcV_dat11_1= LEHRCS_N(:,7);
f_R.tf_R_dat11_1 =LEHRCS_Ww(:,9);

% 1200 ppm a.i.
par_CS = [k_e, c_0, c_T, c_0s, b, h_0, conc.tWw_fe_dat11_2, Fs, t0_c, F]; % pack tox parameters + delay exposure

[t1, LEHRCS] = call_LEHRCS([t], LEHRCS_0, options,  ingestion.tWw_fe_dat11_2,par_LEHR, par_CS);
LEHRCS_Ww = LEHRCS(ismember(t1, tWw_fe_dat11_0(:,1)),:); % Extract growth time points
prdData.tWw_fe_dat11_2 = LEHRCS_Ww(:,1).^3 + w_E/ mu_E/ d_E * (LEHRCS_Ww(:,2))+ w_E/ mu_E * (LEHRCS_Ww(:,4)+LEHRCS_Ww(:,5)); % g, wet weight over time

LEHRCS_N = LEHRCS(ismember(t1, tN_dat11_2(:,1)),:);
prdData.tN_dat11_2 = LEHRCS_N(:,6); 
cV.tcV_dat11_2= LEHRCS_N(:,7);
f_R.tf_R_dat11_2 =LEHRCS_Ww(:,9);

% 3000 ppm a.i.
par_CS = [k_e, c_0, c_T, c_0s, b, h_0, conc.tWw_fe_dat11_3, Fs, t0_c, F]; % pack tox parameters + delay exposure
[t1, LEHRCS] = call_LEHRCS([t], LEHRCS_0, options, ingestion.tWw_fe_dat11_3, par_LEHR, par_CS);
LEHRCS_Ww = LEHRCS(ismember(t1, tWw_fe_dat11_0(:,1)),:); % Extract growth time points
prdData.tWw_fe_dat11_3 = LEHRCS_Ww(:,1).^3 + w_E/ mu_E/ d_E * (LEHRCS_Ww(:,2))+ w_E/ mu_E * (LEHRCS_Ww(:,4)+LEHRCS_Ww(:,5)); % g, wet weight over time

LEHRCS_N = LEHRCS(ismember(t1, tN_dat11_3(:,1)),:);
prdData.tN_dat11_3 = LEHRCS_N(:,6);
cV.tcV_dat11_3= LEHRCS_N(:,7);
f_R.tf_R_dat11_3 =LEHRCS_Ww(:,9);
...
c = par_CS(7);  % mg/g, external concentration of a.i. in the food
...
elseif length(par_LEHR) == 24
    l_b = par_LEHR(22); % -, scaled length at birth
    E_Hj = par_LEHR(23); % J, maturity at metamorphosis
    l_j = par_LEHR(24); % -, scaled length at metamorphosis
