function [prdData, info, f_R, cV] = predict_Colinus_virginianus(par, data, auxData)
  ...
  if E_Hx > E_Hp || f_tW > 1 || f_tW < 0 || t_0 < 0 || t_0 > 7 || ...
          k_e < 0 || b < 0 || c_0s < 0 || c_0 < 0 || c_T < 0 || f_12 < 0 || ... 
          f_12 > 1.5 || kap < 0 || kap > 1
  ...
  TC_tWw_fe_dat12 = tempcorr(C2K(38.9), T_ref, T_A); % consider typical body temperature
...
U_E0 = initial_scaled_reserve(f_mo, pars_UE0);
...
par_LEHR = [pT_M, vT, kT_J, pT_Am, kap, E_G,f, E_0, E_Hb, E_Hp, kap_G, kap_R, L_b, t_s, kap_X, mu_X, d_X, w_X, w_E, mu_E,d_E   ]; % pack parameters
...
par_CS = [k_e, c_0, c_T, c_0s, b, h_0, conc.tWw_fe_dat12_0, Fs, t0_c, F]; % pack tox parameters + delay exposure
...
cV.tcV_dat12_0= LEHRCS_N(:,7);
f_R.tf_R_dat12_0=LEHRCS_Ww(:,9); 

% 500 ppm a.i.
par_CS = [k_e, c_0, c_T, c_0s, b, h_0, conc.tWw_fe_dat12_1, Fs, t0_c, F]; % pack tox parameters + delay exposure
[t1, LEHRCS] = call_LEHRCS([t], LEHRCS_0, options, ingestion.tWw_fe_dat12_1, par_LEHR, par_CS);
LEHRCS_Ww = LEHRCS(ismember(t1, tWw_fe_dat12_1(:,1)),:); % Extract growth time points
prdData.tWw_fe_dat12_1 = LEHRCS_Ww(:,1).^3 + w_E/ mu_E/ d_E * (LEHRCS_Ww(:,2))+ w_E/ mu_E * (LEHRCS_Ww(:,4)+LEHRCS_Ww(:,5)); % g, wet weight over time

LEHRCS_N = LEHRCS(ismember(t1, tN_dat12_1(:,1)),:);
prdData.tN_dat12_1 = LEHRCS_N(:,6);
cV.tcV_dat12_1= LEHRCS_N(:,7);
f_R.tf_R_dat12_1=LEHRCS_Ww(:,9); 

% 1200 ppm a.i.
par_CS = [k_e, c_0, c_T, c_0s, b, h_0, conc.tWw_fe_dat12_2, Fs, t0_c, F]; % pack tox parameters + delay exposure
[t1, LEHRCS] = call_LEHRCS([t], LEHRCS_0, options, ingestion.tWw_fe_dat12_2, par_LEHR, par_CS);
LEHRCS_Ww = LEHRCS(ismember(t1, tWw_fe_dat12_2(:,1)),:); % Extract growth time points
prdData.tWw_fe_dat12_2 = LEHRCS_Ww(:,1).^3 + w_E/ mu_E/ d_E * (LEHRCS_Ww(:,2))+ w_E/ mu_E * (LEHRCS_Ww(:,4)+LEHRCS_Ww(:,5)); % g, wet weight over time

LEHRCS_N = LEHRCS(ismember(t1, tN_dat12_2(:,1)),:);
prdData.tN_dat12_2 = LEHRCS_N(:,6); 
cV.tcV_dat12_2= LEHRCS_N(:,7);
f_R.tf_R_dat12_2=LEHRCS_Ww(:,9); 


% 3000 ppm a.i.
par_CS = [k_e, c_0, c_T, c_0s, b, h_0, conc.tWw_fe_dat12_3, Fs, t0_c, F]; % pack tox parameters + delay exposure
[t1, LEHRCS] = call_LEHRCS([t], LEHRCS_0, options, ingestion.tWw_fe_dat12_3, par_LEHR, par_CS);
LEHRCS_Ww = LEHRCS(ismember(t1, tWw_fe_dat12_3(:,1)),:); % Extract growth time points
prdData.tWw_fe_dat12_3 = LEHRCS_Ww(:,1).^3 + w_E/ mu_E/ d_E * (LEHRCS_Ww(:,2))+ w_E/ mu_E * (LEHRCS_Ww(:,4)+LEHRCS_Ww(:,5)); % g, wet weight over time

LEHRCS_N = LEHRCS(ismember(t1, tN_dat12_3(:,1)),:);
prdData.tN_dat12_3 = LEHRCS_N(:,6); 
cV.tcV_dat12_3= LEHRCS_N(:,7);
f_R.tf_R_dat12_3=LEHRCS_Ww(:,9); 
...
F= par_CS(10);% -, bioavailability of the compound
...
elseif length(par_LEHR) == 24
    ...
    error('par_LEHR must be of length 21 (without acceleration) or 24 (with acceleration; lb, E_Hj, and lj) added)');
end
...
if t > t_s
    if E1_R >= (E0/kap_R)
        ...
        dE1_R = (kap_R * ((1 - kap) * pCm  - k_J * E_Hp + (f_R*p_Am*L2))) * (E0_R > 0) ; % Only if unripped buffer is not 0 and if the condition to produce eggs is met
        dN     = 0 ;
    end
else
    dE1_R = 0; 
...
WwX = spline1(t,tJX);  % g Wt /d, ingestion rate at time t
IQ= WwX * c ; % mg a.i./d,ingestion rate of a.i. at time t 
Ww= L^3 + (w_E/ mu_E/ d_E) * (E)+ (w_E/ mu_E) * (E0_R+E1_R); % g ,wet weight at time t 
dcV = F*IQ/Ww - ke  *  cV - cV *(1 / Ww) * (3*L2*dL+w_E/ mu_E * (dE0_R+dE1_R)+ w_E/ mu_E/ d_E * dE) ; % change in scaled internal conc.(mg/gWwt)
%dcV = F*IQ/Ww - L_M/L*ke  *  cV - cV *(1 / Ww) * (3*L2*dL+w_E/ mu_E * (dE0_R+dE1_R)+ w_E/ mu_E/ d_E * dE) ; % change in scaled internal conc.(mg/gWwt)
%%
% Pack output
